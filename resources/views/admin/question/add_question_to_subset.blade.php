        ...
        <div class="row">
            ...
            <div class="col-lg-6">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        @include('main.error')
                        <form action="{{ route('add.question.to.subset', $subset->id) }}" method="POST">
                            @csrf
                            @method('post')
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control select2" name="questions[]" multiple="multiple"
                                        ...
                                        @foreach (App\Models\Question::all() as $question)
                                            <option
                                                {{ in_array($question->id,old('questions',$subset->questions()->pluck('id')->toArray()))? 'selected': '' }}
                            ...
                            <div class="row justify-content-end">
                                <div class="col-sm-10">
                                    ...
                                    <a class="btn btn-warning" href="{{ route('subset.index') }}">برگشت</a>
                                </div>
                            </div>
                        </form>
                    </div>
                        ...
                        <tbody class="table-border-bottom-0">
                            @foreach ($subset->questions as $question)
                                <tr>
                                    ...
                                    <td>
                                        <span data-id="{{ $subset->id }}" data-question="{{ $question->id }}"
                                            class="alert pointer change_subset_required alert-solid-{{ $question->pivot->required ? 'success' : 'danger' }}">
