@extends('main.manager')
   ...
   <div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card">
        <div class="card-body">
            <form class="dt_adv_search" method="get" action="{{ route('comment.index') }}">
                @csrf
                @method('get')
              <div class="row">
                <div class="col-12">
        ...
        <table class="table ">
            <thead class="table-dark">
                <tr>
                    ...
                    <th> <span class="tit"> نظر گیرنده  </span></th>
                    <th> <span class="tit"> نظر دهنده  </span></th>
                    <th> <span class="tit"> نظر  </span></th>
            ...
            <tbody class="table-border-bottom-0">
                @foreach ($comments as $comment)
                    ...
                    <tr>
                        ...
                        <td><span>{{ Morilog\Jalali\Jalalian::forge($comment->created_at)->format('Y-m-d') }}</span>

                        <td>
                            @if(!$comment->confirm )
                            <form class="inline-block" action="{{ route('confirm.comment', $comment->id) }}"
                                method="post">
                                @csrf
                                @method('post')
                            ...
                            @else
                            {{ Morilog\Jalali\Jalalian::forge($comment->confirm)->format('Y-m-d') }}
                            تایید شده
                            @endif
                        </td>
