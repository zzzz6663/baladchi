...
@if ($user->balance < $advertise->show_option('least_price'))
    <a href="#" class="mony_less" data-ba="{{ $user->balance }}" data-mo="{{ $advertise->show_option('least_price') }}">
        @else
                    ...
                    <div class="single-ads {{ $advertise->check_seen() ? 'seen' : '' }} single_gggg ">
                        ...
                        <div class="img ad_img">
                            ...
                            @if ($user->balance < $advertise->show_option('least_price'))
                                <a href="#" class="mony_less" data-ba="{{ $user->balance }}"
                                    data-mo="{{ $advertise->show_option('least_price') }}"> <img
                                        src="{{ $advertise->first_image() }}" alt=""></a>
                                @else
                                <a href="{{ route('single.ad', $advertise->id) }}">
                                ...
                                <button
                                    class="like-btn fave_f check_fave {{ in_array($advertise->id, $user_fave) ? 'active' : '' }}"
                                    data-id="{{ $advertise->id }}">
                                    <svg class="fave_f" data-id="{{ $advertise->id }}" width="20" height="19"
                        ...
                        <div class="title">
                            <h4>
                                ...
                                @if ($user->balance < $advertise->show_option('least_price'))
                                    <a href="#" class="mony_less" data-ba="{{ $user->balance }}"
                                        data-mo="{{ $advertise->show_option('least_price') }}">{{ $advertise->title
                                        }}</a>
                                    @else
