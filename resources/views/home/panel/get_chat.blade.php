    ...
    <div class="single-contact">
        <div class="img">
            @if ($user->id == $advertise->user->id)
                <img src="{{ $advertise->visitor($uni)->avatar() }}" alt="">
            @else
                <img src="{{ $advertise->user->avatar() }}" alt="">
            @endif

        </div>
        <div class="info">
            @if ($user->id == $advertise->user->id)
                <h4> {{ $advertise->visitor($uni)->name }} {{ $advertise->visitor($uni)->family }} </h4>
            @else
                <h4> {{ $advertise->user->name }} {{ $advertise->user->family }} </h4>
            @endif
            <span class="job">

                @if ($user->id == $advertise->user->id)
                    @foreach ($advertise->visitor($uni)->experts as $expert)
                        {{ $expert->name }}
                    @endforeach
                @else
                    @foreach ($advertise->user->experts as $expert)
                        {{ $expert->name }}
                    @endforeach
                @endif
            </span>
...
<div class="room">
    ...
    @foreach ($advertise->chats()->where('uni', $uni)->oldest()->get() as $chat)
        <div class="chat-boxe {{ $chat->user->id == $user->id ? 'right' : 'left' }}">
            <div class="img">
            ...
            <div class="chat">
                <div class="text" id="chat_cl_{{ $chat->id }}">
                    ...
                    {{--  @if ($user->id == $chat->advertise->visitor($uni)->id)

                    @endif

                    @if ($user->id == $chat->advertise->user->id)
...
<div class="chat-writing">
    <form action="">
        ...
        <textarea name="" data-name="{{ auth()->user()->name }} {{ auth()->user()->family }}"
            data-id="{{ $uni }}" id="send_chat" cols="30" rows="10" placeholder="پیام خود را بنویسید ..."></textarea>
        <span data-uni="{{ $uni }}" class="send send_chat pointer " data-id="{{ $advertise->id }}">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
